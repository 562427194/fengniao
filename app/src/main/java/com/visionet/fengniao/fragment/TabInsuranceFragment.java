...
public class TabInsuranceFragment extends TabFragment {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.fragment_tab_insurance, container, false);
        unbinder = ButterKnife.bind(this, view);
        LinearLayoutManager llm = new LinearLayoutManager( getContext() );
        llm.setOrientation( LinearLayoutManager.VERTICAL );
        rvList.setLayoutManager( llm );
        rvList.setItemAnimator( new DefaultItemAnimator() );
        adapter = new InsuranceAdapter(Data.INSURANCES);
        adapter.addHeaderView(inflater.inflate(R.layout.layout_demo_text,null));
        rvList.setAdapter(adapter);
        adapter.setOnItemClickListenner(new BaseListAdapter.OnItemClickListenner<Insurance>() {
            @Override
            public void onItemClick(int position, Insurance insurance) {
                InsuranceDetailActivity.toInsuranceDetailActivity(getContext(),insurance);
            }
        });
